/*
 * Copyright (C) 2000-2002, 2004, 2005, 2006, Jason L. Shiffer
 * <jshiffer@zerotao.com>.  All Rights Reserved. See file COPYING for details.
 *
...
static void intlen_test(struct zt_unit_test *test, void *data UNUSED) {
  int volatile i;
  int failures = 0;
  ...
  failures = 0;
  for (i = -100; i > -1000; i--) {
    if (zt_cstr_int_display_len(i) != 4) {
      failures++;
    }
  }
...
int register_cstr_suite(struct zt_unit *unit) {
