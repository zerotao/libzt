...
#define test_decoding(_base, _data, _len, _result1, _result2)               \
    do { char            result[20];                                        \
         ...
         memset(result, 0, sizeof(result));                                 \
         len = sizeof(result);                                              \
         ret = zt_base_decode(_base, (unsigned char *)_result1, strlen(_result1), result, &len);                  \
         ZT_UNIT_ASSERT(test, !ret);                                        \
         ...
         memset(result, 0, sizeof(result));                                    \
         len = sizeof(result);                                     \
         ret = zt_base_decode(_base, _result2, strlen(_result2), result, &len); \
         ZT_UNIT_ASSERT(test, !ret);                            \
         ZT_UNIT_ASSERT(test, strcmp(result, (char *)_data) == 0);      \
         ZT_UNIT_ASSERT(test, strlen(result) == len);              \
    } while (0)
...
decoding_tests(struct zt_unit_test * test, void * _data UNUSED) {
    ...
    ret = zt_base_decode(zt_base64_rfc, "Zg", strlen("Zg"), result, NULL);
    ZT_UNIT_ASSERT(test, ret == -1);
    ...
    /* check insufficient output space */
    len = 1;
    ret = zt_base_decode(zt_base64_rfc, "Zm8", strlen("Zm8"), result, &len);
    ZT_UNIT_ASSERT(test, ret == -2);
    ZT_UNIT_ASSERT(test, len == 3);

    /* check missing input */
    len = 2;
    ret = zt_base_decode(zt_base64_rfc, NULL, 0, result, &len);
    ZT_UNIT_ASSERT(test, ret == 0);
    ZT_UNIT_ASSERT(test, len == 0);

    /* check empty input */
    len = 2;
    ret = zt_base_decode(zt_base64_rfc, "", 0, result, &len);
    ZT_UNIT_ASSERT(test, ret == 0);
    ZT_UNIT_ASSERT(test, len == 0);
