/*
 * Copyright (C) 2008, Jason L. Shiffer <jshiffer@zerotao.org>.
 * All Rights Reserved.
...
zt_sha1_finalize(zt_sha1_ctx *ctx, uint8_t digest[20])
{
    ...
    zt_sha1_update(ctx, (uint8_t *)"\200", 1);
    while ((ctx->count[0] & 504) != 448) {
        zt_sha1_update(ctx, (uint8_t *)"\0", 1);
    }
	
    zt_sha1_update(ctx, finalcount, 8);  /* Should cause a SHA1Transform() */
    for (i = 0; i < 20; i++) {
        digest[i] = (uint8_t)
...
void
zt_sha1_data(void *data, size_t len, uint8_t digest[20]) 
{
	zt_sha1_ctx		  ctx;
	
	zt_sha1_init(&ctx);
	zt_sha1_update(&ctx, data, len);
